...
import "./IDelegableTokenController.sol";
import "./DelegableMiniMeToken.sol";


contract DelegableTokenManager is IDelegableTokenController, IForwarder, AragonApp {
    ...
    // Note that we COMPLETELY trust this DelegableMiniMeToken to not be malicious for proper operation of this contract
    DelegableMiniMeToken public token;
    ...
    * @param _token DelegableMiniMeToken address for the managed token (Token Manager instance must be already set as the token controller)
    * @param _transferable whether the token can be transferred by holders
    * @param _maxAccountTokens Maximum amount of tokens an account can have (0 for infinite tokens)
    */
    function initialize(
        DelegableMiniMeToken _token,
        bool _transferable,
        bool _delegable,
        ...
        onlyInit
    {
        ...
        if (token.delegationEnabled() != _delegable) {
            token.enableDelegation(_delegable);
        }
    }
    ...
    */
    ...
    /**
    * @dev Notifies the controller about a delegation allowing the controller to react if desired
    *      Initialization check is implicitly provided by `onlyToken()`.
    * @return False if the controller does not authorize the approval
    */
    function onDelegate(address, address, uint) external onlyToken returns (bool) {
        return true;
    }

    /**
    * @dev Notifies the controller about an undelegation allowing the controller to react if desired
    *      Initialization check is implicitly provided by `onlyToken()`.
    * @return False if the controller does not authorize the approval
    */
    function onUnDelegate(address, address, uint) external onlyToken returns (bool) {
        return true;
    }
